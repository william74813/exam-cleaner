(() => {
  ...
  if (heading) heading.textContent = 'Exam Cleaner v2.2 Alpha.5';
  if (badge) badge.textContent = 'OpenAI 連線驗證';

  const proxyOption = mode.querySelector('option[value="proxy"]');
  if (proxyOption) proxyOption.textContent = 'OpenAI 安全 Proxy（需先測試連線）';

  const testButton = document.createElement('button');
  testButton.id = 'testProxyConnection';
  testButton.type = 'button';
  testButton.className = 'orange';
  testButton.textContent = '測試 OpenAI Proxy 與權杖';
  ...
  connectionStatus.textContent = '尚未驗證。測試不會上傳圖片，也不會呼叫 OpenAI 圖片模型。';
  ...
  function normalizedEndpoint() {
    ...
    if (!url.pathname.endsWith('/api/clean')) throw new Error('Proxy URL 應以 /api/clean 結尾');
    url.search = '';
    url.hash = '';
    return url;
  }
  ...
  function invalidate(message = '設定已變更，請重新測試 OpenAI Proxy 與權杖。') {
    verifiedKey = '';
  ...
  function updateModeText() {
    if (mode.value === 'proxy') {
      if (modeHelp) modeHelp.textContent = '圖片只會送至你設定的安全 Proxy；OpenAI API Key 保存在 Worker Secret，不會進入手機網頁。';
      processButton.textContent = verified() ? '開始 OpenAI 去筆跡' : '請先測試 OpenAI Proxy';
    }
  }
  ...
  function enforce() {
    if (mode.value === 'proxy' && !verified()) {
      ...
      processButton.textContent = '請先測試 OpenAI Proxy';
    } else if (mode.value === 'proxy') {
      processButton.textContent = '開始 OpenAI 去筆跡';
    }
    updateModeText();
  }
  ...
  testButton.addEventListener('click', async () => {
    ...
    setConnectionStatus('正在檢查 Worker、OpenAI Secret 與存取權杖……', 'warn');
    enforce();
    ...
    try {
      ...
      if (health.provider !== 'openai') throw new Error('此 Worker 仍是舊供應商版本，請重新部署 OpenAI Proxy');
      if (health.apiVersion !== 'exam-clean-v2') throw new Error('Proxy API 版本不相容');
      if (!health.ready) throw new Error('Worker 尚未設定完整的 OpenAI API Key 與存取權杖');
      ...
      if (auth.provider !== 'openai') throw new Error('權杖連到的不是 OpenAI Proxy');

      verifiedKey = currentKey();
      sessionStorage.setItem('examCleanerProxy', endpoint.value);
      sessionStorage.setItem('examCleanerProxyToken', token.value);
      setConnectionStatus(`✓ OpenAI Proxy 與權杖驗證成功；模型：${auth.model || health.model || '已設定'}。`, 'success');
    } catch (error) {
      verifiedKey = '';
  ...
  mode.addEventListener('change', () => {
    if (mode.value === 'proxy' && !verified()) {
      setConnectionStatus('請先輸入 OpenAI Proxy URL 與存取權杖，再執行連線測試。', 'warn');
    }
  ...
  processButton.addEventListener('click', event => {
    ...
    setConnectionStatus('尚未通過 OpenAI Proxy 與權杖驗證，不能上傳考卷。', 'danger');
    enforce();
  }, true);
